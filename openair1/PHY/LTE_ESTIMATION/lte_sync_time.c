...
int lte_sync_time_init(LTE_DL_FRAME_PARMS *frame_parms ) { // LTE_UE_COMMON *common_vars
  ...
  for (int i = 0; i < sizeofArray(primary_synch_time); i++) {
    AssertFatal(NULL != (primary_synch_time[i] = (c16_t *)malloc16(sz)), "");
    bzero(primary_synch_time[i], sz);
  }
...
void lte_sync_time_free(void) {
  for (int i = 0; i < sizeofArray(primary_synch_time); i++)
    if (primary_synch_time[i]) {
      ...
      free(primary_synch_time[i]);
      primary_synch_time[i] = NULL;
    }
}
...
int lte_sync_time_eNB(int32_t **rxdata, ///rx data in time domain
  ...
  for (n=0; n<length; n+=4) {
    ...
    if (n<(length-frame_parms->ofdm_symbol_size-frame_parms->nb_prefix_samples)) {
      //calculate dot product of primary_synch0_time and rxdata[ar][n] (ar=0..nb_ant_rx) and store the sum in temp[n];
      for (ar=0; ar<frame_parms->nb_antennas_rx; ar++)  {
        c32_t result = dot_product(primary_synch_time[eNB_id], (c16_t *)&(rxdata[ar][n]), frame_parms->ofdm_symbol_size, SHIFT);
        //((short*)sync_corr)[2*n]   += ((short*) &result)[0];
        //((short*)sync_corr)[2*n+1] += ((short*) &result)[1];
        sync_corr_eNB[n] += squaredMod(result);
      }
    }
...
int ru_sync_time(RU_t *ru,
  ...
  for (int n=0; n<length; n+=4) {
    ...
    //calculate dot product of primary_synch0_time and rxdata[ar][n] (ar=0..nb_ant_rx) and store the sum in temp[n];
    for (int ar=0; ar<ru->nb_rx; ar++) {
      c32_t result = dot_product((c16_t *)ru->dmrssync, (c16_t *)&ru->common.rxdata[ar][n], frame_parms->ofdm_symbol_size, shift);

      if (ru->state == RU_CHECK_SYNC) {
        result = dot_product((c16_t *)&calibration->drs_ch_estimates_time[ar], (c16_t *)&ru->common.rxdata[ar][n], frame_parms->ofdm_symbol_size, shift);
      }

      dmrs_corr += squaredMod(result);
    }
