...
/* Form definition file generated by Bo Zhao (Fraunhofer IIS)
   Scope implemented by Marwan Hammouda (Fraunhofer IIS)*/
...
/* @gNB: This class creates the drop-down list at the UE side. Each item correspinds to an implemented KPI. */
...
/* @gNB: This class creates the window when choosing the option 'Configs' at the gNB side to configure the threshold values. */
...
/* @gNB: This function reads the input config values, entered by user,  and update the array Limits_KPI_gNB accordignly. */
...
// @UE: This class creates the window when choosing the option 'Configs' at the UE side to configure the threshold values
...
// @UE: This function reads the input config values, entered by user, and update the array Limits_KPI_ue accordignly
...
PainterWidgetgNB::PainterWidgetgNB(QComboBox *parent, scopeData_t *p)
{
    ...
    makeConnections();   // check which KPI has been selected to display
}

/* @gNB Class: This function resets the series carrying the data to display. This function will be called
   by the KPIs showing acculative (dynamic) data, and it will be called whenever the current series should be cleared
   1. when choosing the KPI for the first time to be plotted in the current widget
   2. when the curve reached the limit on the x-axis, and it should start from zero again on the x-axis. */
...
/* @gNB Class: This function resets the max, min, and avg. values of dynamic series. This function is called only when
   choosing the KPI for the first time in the current widget.  */
...
/* @gNB: when the update() function is called to paint on the widget. This is particularly called when drawing on a pix map. */
...
/* @gNB: this function to plot the waterfall graph for the RX signal in time domain for one frame. x-axis shows the frame divided into slots
...
/* @gNB: function called when the configuration option is chosen. */
...
/* @gNB: this function plots the KPI of the number of re-retranmissions on the DL. x-axis is the time index, and four curves will be displayed
   corresponding to the re-transmissions of the first four rounds. */
void PainterWidgetgNB::KPI_DL_Retrans()
{
  ...
  if ((this->DLRetrans[0].plot_idx > this->chartWidth) ||    // if the x-axis approaches its limit, or this KPI is chosen for the first time
      (this->indexToPlot != this->previousIndex))
  {
    this->DLRetrans[0].plot_idx = 0;
    this->chart->removeAllSeries();    // clear all previous series from the current chart
    for (int i=0; i<4; i++)
    {
      resetKPIPlot(&this->DLRetrans[i]);
      resetKPIValues(&this->DLRetrans[i]);
...
/* @gNB: this function plots the KPI of the number of re-retranmissions on the UL. x-axis is the time index, and four curves will be displayed
   corresponding to the re-transmissions of the first four rounds. */
void PainterWidgetgNB::KPI_UL_Retrans()
{
  ...
  if ((this->ULRetrans[0].plot_idx > this->chartWidth) ||    // if the x-axis approaches its limit, or this KPI is chosen for the first time
      (this->indexToPlot != this->previousIndex))
  {
    this->ULRetrans[0].plot_idx = 0;
    this->chart->removeAllSeries();   // remove all previous series from the current chart
    for (int i=0; i<4; i++)
    {
      resetKPIPlot(&this->ULRetrans[i]);
      resetKPIValues(&this->ULRetrans[i]);
...
void PainterWidgetgNB::KPI_DL_SNR()
{
  ...
  initialCheck(&this->DLSNR, false);   // check if KPI series and/or should be reset
  ...
  const QString xLabel = QString("Time Index");
  const QString yLabel = QString("DL SNR (CQI)");
  createDynamicMap(&this->DLSNR, xLabel, yLabel, 10, true);  // plot function
}
...
void PainterWidgetgNB::KPI_UL_SNR()
{
  ...
  initialCheck(&this->ULSNR, false);   // check if KPI series and/or should be reset
  ...
  const QString xLabel = QString("Time Index");
  const QString yLabel = QString("PUSCH SNR dB");
  createDynamicMap(&this->ULSNR, xLabel, yLabel, 10, true);
}
...
void PainterWidgetgNB::KPI_Nof_RBs()
{
  ...
  initialCheck(&this->nofRBs, false);  // check if KPI series and/or should be reset
  ...
  const QString xLabel = QString("Time Index");
  const QString yLabel = QString("Nof Scheduled RBs");
  createDynamicMap(&this->nofRBs, xLabel, yLabel, 10, true);  // plot function
}

// @gNB: DL throughput in Mbs
void PainterWidgetgNB::KPI_DL_Throu()
{
  ...
  float bler_dl = (float)sched_ctrl->dl_bler_stats.bler;   // block error rate
  NR_DL_FRAME_PARMS *frame_parms = &this->p->gNB->frame_parms;
  ...
  double blockSizeBits = (double)(blockSize << 3);      // block size in bits
  double ThrouputKBitSec = blerTerm * blockSizeBits / (double)slotDuration;    // throughput in kBit/sec

  initialCheck(&this->DLThrou, false);  // check if KPI series and/or should be reset
  ...
  const QString xLabel = QString("Time Index");
  const QString yLabel = QString("DL Throughput Mbit/sec");
  createDynamicMap(&this->DLThrou, xLabel, yLabel, 3, true);
}
...
void PainterWidgetgNB::KPI_UL_Throu()
{
  ...
  float bler_ul = (float)sched_ctrl->ul_bler_stats.bler;    // block error rate
  ...
  initialCheck(&this->ULThrou, false);  // check if KPI series and/or should be reset
  ...
  const QString xLabel = QString("Time Index");
  const QString yLabel = QString("UL Throughput Mbit/sec");
  createDynamicMap(&this->ULThrou, xLabel, yLabel, 1, true);  // plot function
}
...
void PainterWidgetgNB::KPI_DL_MCS()
{
  ...
  initialCheck(&this->DLMCS, true);  // check if KPI series and/or should be reset
  ...
  const QString xLabel = QString("Time Index");
  const QString yLabel = QString("DL MCS");
  createDynamicMap(&this->DLMCS, xLabel, yLabel, -1, true);  // Plot function
}
...
void PainterWidgetgNB::KPI_DL_BLER()
{
  ...
  initialCheck(&this->DLBLER, false);   // check if KPI series and/or should be reset
  ...
  const QString xLabel = QString("Time Index (calc window: 100 ms)");
  const QString yLabel = QString("DL BLER");
  createDynamicMap(&this->DLBLER, xLabel, yLabel, 2, false);
}

/* @gNB: this function checks if the KPI series or values should be reset
   inputStruct [in/out]: structure containing the series of this KPI.
   maxMin [in]: bool variable indicating if min./max. values are also to be plotted. */
void PainterWidgetgNB::initialCheck(KPI_elements *inputStruct, bool maxMin)
{
  if ((inputStruct->plot_idx > this->chartWidth) ||  // check if this KPI is chosen for the first time,
      (this->indexToPlot != this->previousIndex))    // or if the x-axis exceeding the widget width
  {
    inputStruct->plot_idx = 0;
    this->chart->removeAllSeries();   // clear the current chart by removing all previous series
    resetKPIPlot(inputStruct);        // reset series
  }
  if((this->indexToPlot != this->previousIndex) && (maxMin))   // if this KPI is chosen for first time and max., min. values to be plotted
    resetKPIValues(inputStruct);         // reset values
}

/* @gNB: This function plots the series carrying dynamic data, i.e., the series data are added with time.
   inputStruct[in]: structure of series containing data to be plotted
   xLabel[in]: text to be displayed on the x axis
   yLabel[in]: text to be displayed on the y axis
   thresholdIdx[in]: index to access the threshold values from the array Limits_KPI_gNB.
   limitY[in]: bool variable indicating if the y axis should be scaled with respect to the data, otherwise it will be set to a fixed limits. */
void PainterWidgetgNB::createDynamicMap(KPI_elements *inputStruct, const QString xLabel, const QString yLabel, int thresholdIdx, bool limitY)
{
  this->chart->removeAxis(this->axisX);
  this->chart->removeAxis(this->axisY);
  if (thresholdIdx != -1)
  ...
  this->axisY->setTickCount(nofTicks);
  if (limitY)
    this->axisY->setRange(-1, inputStruct->max_value + 2.0);
  else
    this->axisY->setRange(-1, 1.5);
  this->axisY->setTitleText(yLabel);
  this->chart->addAxis(this->axisY, Qt::AlignLeft);

  if (thresholdIdx < 0)    // KPI that do not have threshold values to be plotted
  {
    if(inputStruct->plot_idx == 0)
      this->chart->addSeries(inputStruct->series);
    inputStruct->series->attachAxis(this->axisX);
    inputStruct->series->attachAxis(this->axisY);

    if(inputStruct->plot_idx == 0)
      this->chart->addSeries(inputStruct->seriesMin);
    inputStruct->seriesMin->attachAxis(this->axisX);
    inputStruct->seriesMin->attachAxis(this->axisY);

    if(inputStruct->plot_idx == 0)
      this->chart->addSeries(inputStruct->seriesMax);
    inputStruct->seriesMax->attachAxis(this->axisX);
    inputStruct->seriesMax->attachAxis(this->axisY);
  ...
  else
  {
    ...
    if (thresholdIdx < 5)   // valid threshold index
    {
      QLineSeries *series_LowLim = new QLineSeries();
      series_LowLim->append(0, Limits_KPI_gNB[thresholdIdx][0]);
      series_LowLim->append(this->chartWidth, Limits_KPI_gNB[thresholdIdx][0]);
      series_LowLim->setColor(QColor(0, 255, 0));

      QLineSeries *series_UppLim = new QLineSeries();
      series_UppLim->append(0, Limits_KPI_gNB[thresholdIdx][1]);
      series_UppLim->append(this->chartWidth, Limits_KPI_gNB[thresholdIdx][1]);
...
void PainterWidgetgNB::KPI_UL_BLER()
{
  ...
  initialCheck(&this->ULBLER, false);
  ...
  const QString xLabel = QString("Time Index (calc window: 100 ms)");
  const QString yLabel = QString("UL BLER");
  createDynamicMap(&this->ULBLER, xLabel, yLabel, 0, false);  // y axis will not be scaled, as the BLER can not exceed 1
}
...
void PainterWidgetgNB::KPI_UL_MCS()
{
  ...
  const QString xLabel = QString("Time Index");
  const QString yLabel = QString("UL MCS");
  createDynamicMap(&this->ULMCS, xLabel, yLabel, -1, true);
}

/* @gNB: PUSCH I/Q Constellation
   Here, no dynamic series will be used, as new data will be displayed at each instance .*/
...
// @gNB: PUSCH LLR. also here no dynamic series
...
/* @gNB: plot static (non-dynamic) series. This function is mainly called for IQ and LLR plots.
   xData[in]: pointer to data to be displayed on the x-axis
   yData[in]: pointer to data to be displayed on the y-axis
   len[in]: length of the data to be displayed
   MarkerColor[in]: plotting color
   xLabel[in]: text to ne displayed on the x-axis
   yLabel[in]: text to ne displayed on the y-axis
   scaleX[in] boolian value indicating if x-axis to be scaled with respect to data. This will be set mainly for IQ plots. */
...
/* @UE: waterfall graph for the received signal in time domain over one frame.
  The x-axis represents one frame with vertical lines corresponding to the slots.
  The y-axis is a color map of the SquaredNorm of the signall at the corresponding slot. */
...
/* @UE Class: This function resets the series carrying the data to display. This function will be called
   by the KPIs showing acculative (dynamic) data, and it will be called whenever the current series should be cleared
   1. when choosing the KPI for the first time to be plotted in the current widget
   2. when the curve reached the limit on the x-axis, and it should start from zero again on the x-axis. */
...
/* @UE Class: This function resets the max, min, and avg. values of dynamic series. This function is called only when
   choosing the KPI for the first time in the current widget.  */
...
/* @UE: plot static (non-dynamic) scatter series. This function is mainly called for IQ and LLR plots.
   xData[in]: pointer to data to be displayed on the x-axis
   yData[in]: pointer to data to be displayed on the y-axis
   len[in]: length of the data to be displayed
   MarkerColor[in]: plotting color
...
// @UE: this KPI shows the frequency offset and timing advance
...
void PainterWidget::KPI_Nof_RBs()
{
  ...
  initialCheck(&this->nofRBs, false);
...
void PainterWidget::KPI_DL_Throu()
{
  ...
  const QString xLabel = QString("Time Index");
  const QString yLabel = QString("UL Throughput Mbit/sec");
  createDynamicMap(&this->Throu, xLabel, yLabel, 1, true);
}
...
void PainterWidget::KPI_DL_MCS()
{
  ...
  const QString xLabel = QString("Time Index");
  const QString yLabel = QString("DL MCS");
  createDynamicMap(&this->DLMCS, xLabel, yLabel, -1, true);
}
...
void PainterWidget::KPI_DL_BLER()
{
  ...
  initialCheck(&this->DLBLER, false);
  ...
  const QString xLabel = QString("Time Index");
  const QString yLabel = QString("DL BLER");
  createDynamicMap(&this->DLBLER, xLabel, yLabel, 0, false);  // y axis will not be scaled, as the BLER can not exceed 1
}


/* @UE: This function plots the series carrying dynamic data, i.e., the series data are added with time.
   inputStruct[in]: structure of series containing data to be plotted.
   xLabel[in]: text to be displayed on the x axis
   yLabel[in]: text to be displayed on the y axis
   thresholdIdx[in]: index to access the threshold values from the array Limits_KPI_ue
   limitY[in]: bool variable indicating if the y axis should be scaled with respect to the data, otherwise it will be set to a fixed limits. */
void PainterWidget::createDynamicMap(KPI_elements *inputStruct, const QString xLabel, const QString yLabel, int thresholdIdx, bool limitY)
{
  this->chart->removeAxis(this->axisX);
  this->chart->removeAxis(this->axisY);
  if (thresholdIdx != -1)
  ...
  else
  {
    ...
    {
      QLineSeries *series_LowLim = new QLineSeries();
      series_LowLim->append(0, Limits_KPI_ue[thresholdIdx][0]);
      series_LowLim->append(this->chartWidth, Limits_KPI_ue[thresholdIdx][0]);
      series_LowLim->setColor(QColor(0, 255, 0));

      QLineSeries *series_UppLim = new QLineSeries();
      series_UppLim->append(0, Limits_KPI_ue[thresholdIdx][1]);
      series_UppLim->append(this->chartWidth, Limits_KPI_ue[thresholdIdx][1]);
      series_UppLim->setColor(QColor(255, 0, 0));
      this->chart->addSeries(series_LowLim);
      series_LowLim->attachAxis(this->axisX);
      series_LowLim->attachAxis(this->axisY);
...
/* @UE: this function checks if the KPI series or values should be reset
   inputStruct [in/out]: structure containing the series of this KPI
   maxMin [in]: bool variable indicating if min./max. values are also to be plotted. */
void PainterWidget::initialCheck(KPI_elements *inputStruct, bool maxMin)
{
  if ((inputStruct->plot_idx > this->chartWidth) ||  // check if this KPI is chosen for the first time,
      (this->indexToPlot != this->previousIndex))    // or if the x-axis exceeding the widget width
  {
...
// this function is used to stream data that are not accessible through global variables.
